...
#
# # 判断两个字符串是否相似
# def is_similar(gold_label, concept, threshold=0.1):  # 降低阈值至 0.5
#     similarity = SequenceMatcher(None, gold_label.lower(), concept.lower()).ratio()
#     return similarity >= threshold
#
#
# # Load IFC concepts (entities and relationships)
...
#
#
# # Load your custom JSON dataset, where each line is a JSON object
...
#
#
# # Extract spans based on entity offsets
...
#         span = text[start:end]
#         spans.append({'text': span, 'label': entity['label'], 'start': start, 'end': end})
#     return spans
#
#
# # Train TF-IDF vectorizer using character n-grams on spans and IFC entities
...
#
#
# # Run high-recall matching (HRM) with IFC prioritization and strict filtering
# def run_hrm_with_strict_filtering(spans, ifc_entities, span_vectors, entity_vectors, context_windows, threshold=0.1, top_k=500):  # 增大 top_k
#     matched_entities = []
...
#                     adjusted_score += 0.5  # 增强优先权
#                     adjusted_score = min(adjusted_score, 1.0)  # 限制分数范围
...
#
# # Calculate recall with corrected logic
...
#             gold_label = match['span']  # Use the span text as the gold label
#             for k in k_values:
#                 # 修正逻辑：确保 top_matches 不会少于 k 个
#                 top_matches = match['matches'][:k] if len(match['matches']) >= k else match['matches']
#                 top_concepts = [m['concept'] for m in top_matches]
#
#                 # 检查是否有相似的候选实体
...
#
#
# # Map relationships to IFC concepts with score calculation
...
# def process_custom_dataset_with_context(dataset_path, ifc_entity_file_path, ifc_relation_file_path, stop_words):
...
#
#             # Generate context for spans
#             context_windows = {span['text']: "mock_context" for span in candidate_spans}
#
#             # Context-aware HRM with strict IFC filtering
...
#
# # Define paths
# dataset_path = 'admin2.jsonl'
# ifc_entity_file_path = 'ifc_entities.txt'
# ifc_relation_file_path = 'ifc_relations.json'
#
# stop_words = set(["的", "了", "在", "是", "和"])
#
# process_custom_dataset_with_context(dataset_path, ifc_entity_file_path, ifc_relation_file_path, stop_words)
...
#
# # 判断两个字符串是否相似
# def is_similar(gold_label, concept, threshold=0.05):  # 降低阈值至 0.5
#     similarity = SequenceMatcher(None, gold_label.lower(), concept.lower()).ratio()
#     return similarity >= threshold
#
#
...
#
#
# # Run high-recall matching (HRM) with enhanced filtering and prioritization
# def run_hrm_with_enhancements(spans, ifc_entities, span_vectors, entity_vectors, context_windows, threshold=0.05, top_k=100):
#     matched_entities = []
#     all_scores = cosine_similarity(span_vectors, entity_vectors)
#
#     for i, span in enumerate(spans):
...
#             gold_label = match['span']  # Use the span text as the gold label
#             for k in k_values:
#                 # 确保 matches 数量足够，避免 Recall@5 和 Recall@10 受影响
#                 top_matches = match['matches'][:k] if len(match['matches']) >= k else match['matches']
#                 top_concepts = [m['concept'] for m in top_matches]
#
#                 # 检查是否有匹配的候选实体
...
#             context_windows = {span['text']: "mock_context" for span in candidate_spans}
#
#             # Context-aware HRM with enhanced logic
#             matched_entities = run_hrm_with_enhancements(
#                 candidate_spans, ifc_entities, span_vectors, entity_vectors, context_windows
#             )
#
#             mapped_relations = map_relationships(relations, ifc_relations)
...
#         print(f"Recall metrics for {dataset_name} dataset: {metrics}")
#
#     output_file = 'hrm_with_enhanced_results.json'
#     with open(output_file, 'w', encoding='utf-8') as f:
#         json.dump({'results': results, 'recall_metrics': recall_metrics}, f, ensure_ascii=False, indent=4)
#     print(f'Results and recall metrics saved to {output_file}')
#
...
#     for dataset_type, entries in data["results"].items():
#         print(f"Analyzing {dataset_type} dataset...")
#         for entry in entries:
#             # Count candidate entities for each span
#             for match in entry.get("matched_entities", []):
#                 span = match["span"]
#                 num_candidates = len(match.get("matches", []))
#                 span_entity_counts[span] += num_candidates
#
#             # Count candidate relationships for each mapped relation
#             for relation in entry.get("mapped_relations", []):
#                 mapped_type = relation["mapped_type"]
#                 relation_type_counts[mapped_type] += 1
#
#     # Calculate total counts
#     total_entity_count = sum(span_entity_counts.values())
#     total_relation_count = sum(relation_type_counts.values())
#
#     # Print results
#     print("\n--- Span Candidate Entity Counts ---")
#     for span, count in span_entity_counts.items():
#         print(f"Span: {span}, Candidate Entity Count: {count}")
#     print(f"Total Span Candidate Entity Count: {total_entity_count}")
#
#     print("\n--- Relation Candidate Type Counts ---")
#     for relation, count in relation_type_counts.items():
#         print(f"Relation Type: {relation}, Count: {count}")
#     print(f"Total Relation Candidate Type Count: {total_relation_count}")
#
#     # Return results for further use
#     return span_entity_counts, relation_type_counts, total_entity_count, total_relation_count
#
#
# # Specify the path to the JSON file
# json_file_path = 'hrm_with_shuffled_results.json'
#
# # Analyze the JSON file
# span_entity_counts, relation_type_counts, total_entity_count, total_relation_count = analyze_json_file(json_file_path)
...
def run_hrm_with_strict_filtering(spans, ifc_entities, span_vectors, entity_vectors, context_windows, threshold=0.01, top_k=10):
    ...
    for i, span in enumerate(spans):
        scores = all_scores[i]
        top_indices = np.argsort(scores)[::-1][:top_k]  # 确保 top_k 足够大
        matches = []
...
# 设置本地模型路径
model_path = 'Helsinki-NLP/opus-mt-zh-en'  # 替换为你本地模型的路径
