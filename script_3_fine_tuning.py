...
from code.MethodBGCCNodeClassification import MethodBGCCNodeClassification
from code.ResultSaving import ResultSaving
from code.Settings import Settings
from code.MethodBGCCGraphClustering import MethodBGCCGraphClustering
import numpy as np
import torch
import json
...
#---- Fine-Tuning Task 1: Graph Bert Node Classification (training) ----
if 1:
    #---- hyper-parameters ----
    ...
    print('************ Start ************')
    print('BGCC, dataset: ' + dataset_name + ', residual: ' + residual_type + ', k: ' + str(k) + ', hidden dimension: ' + str(hidden_size) +', hidden layer: ' + str(num_hidden_layers) + ', attention head: ' + str(num_attention_heads))
    ...
    method_obj = MethodBGCCNodeClassification(bert_config)

    #---- set to false to run faster ----
    # method_obj.weight_decay = 0.01
    ...
    method_obj.save_pretrained('./result/PreTrained_BGCC/' + dataset_name + '/node_classification_complete_model/')
    print('************ Finish ************')
#------------------------------------
...
if 1:
    ...
    bert_config = GraphBertConfig(residual_type = residual_type, k=k, x_size=nfeature, y_size=y_size, hidden_size=hidden_size, intermediate_size=intermediate_size, num_attention_heads=num_attention_heads, num_hidden_layers=num_hidden_layers)
    method_obj = MethodBGCCGraphClustering(bert_config)
    #---- set to false to run faster ----
    ...
    result_obj = ResultSaving()
    result_obj.result_destination_folder_path = './result/BGCC/clustering_' + dataset_name
