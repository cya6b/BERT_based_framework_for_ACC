...
from code.MethodBGCCNodeConstruct import MethodBGCCNodeConstruct
from code.MethodBGCCGraphRecovery import MethodBGCCGraphRecovery
...
if 0:
    ...
    print('************ Start ************')
    print('BGCC, dataset: ' + dataset_name + ', Pre-training, Node Attribute Reconstruction.')
    ...
    method_obj = MethodBGCCNodeConstruct(bert_config)
...
if 0:
    ...
    print('************ Start ************')
    print('BGCC, dataset: ' + dataset_name + ', Pre-training, Graph Structure Recovery.')
    # ---- objection initialization setction ---------------
    data_obj = DatasetLoader()
    data_obj.dataset_source_folder_path = './data/' + dataset_name + '/'
    data_obj.dataset_name = dataset_name
    ...
    bert_config = GraphBertConfig(residual_type = residual_type, k=k, x_size=nfeature, y_size=y_size, hidden_size=hidden_size, intermediate_size=intermediate_size, num_attention_heads=num_attention_heads, num_hidden_layers=num_hidden_layers)
    method_obj = MethodBGCCGraphRecovery(bert_config)
    method_obj.max_epoch = max_epoch
    method_obj.lr = lr
