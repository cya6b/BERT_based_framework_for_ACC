...
#--------------- BGCC Pre-Training Records Convergence --------------
...
if 0:
    ...
    result_obj.result_destination_folder_path = './result/BGCC/'
    best_score = {}
    ...
    plt.plot(epoch_list, train_loss, label='BGCC')
...
#         plt.plot(x, train_acc, label='BGCC(' + str(depth) + '-layer)')
#
#     plt.xlim(0, 300)
...
#         plt.plot(x, test_acc, label='BGCC(' + str(depth) + '-layer)')
...
if 0:
    ...
    for depth in depth_list:
        ...
        plt.plot(x[:len(train_acc_smooth)], train_acc_smooth, label='BGCC(' + str(depth) + '-layer)')
    ...
    for depth in depth_list:
        ...
        plt.plot(x[:len(test_acc_smooth)], test_acc_smooth, label='BGCC(' + str(depth) + '-layer)')
    ...
    for depth in depth_list:
        ...
        plt.plot(x[:len(train_loss_smooth)], train_loss_smooth, label='Bert-GE(' + str(depth) + '-layer)')
    ...
    for depth in depth_list:
        ...
        plt.plot(x[:len(test_loss_smooth)], test_loss_smooth, label='BERT-GE(' + str(depth) + '-layer)')
    ...
    # 选择特定的depth
    selected_depth = 2
    epoch_list = range(150)  # 添加这行代码来定义epoch_list
    ...
    # 创建一个新的CSV文件
    with open('epoch_loss_depth_3.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        # 写入列名
        writer.writerow(["Epoch", "Loss"])
        # 写入每个epoch和对应的loss值
        if selected_depth in depth_result_dict:
            for i in range(len(epoch_list)):
                writer.writerow([epoch_list[i], depth_result_dict[selected_depth][i]['loss_test']])



#--------------- BGCC Learning Convergence (Confusion Matrix, accuracy, ROC) --------------
...
if 1:
    ...
    data_obj.dataset_name = 'training'  # 或者你正在使用的数据集名称
    data_obj.dataset_source_folder_path = './data/training/'  # 设置数据集文件的路径
    data_obj.load()
    ...
    # 打印每个标签的精确度、召回率、F1分数、支持度和准确率
    for i in range(3):  # 你有3个类别
        print(
            f"{class_names[i]}: Precision: {precision[i]}, Recall: {recall[i]}, F1: {f1[i]}, Support: {support[i]}, Accuracy: {label_accuracy[i]}")
    ...
    # y_pred_bin = label_binarize(y_pred_nn1, classes=[0, 1, 2])
    #
